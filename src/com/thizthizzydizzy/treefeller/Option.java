...
public abstract class Option<E> {
	public static final HashSet<Material> defaultOverridables = new HashSet<>();
	static {
	...
	public static OptionBoolean STARTUP_LOGS = new OptionBoolean("Startup Logs", true, false, false, false) {
		@Override
	...
	public static Option<Integer> MAX_LOGS = new Option<Integer>("Max Logs", true, true, true, 250) {
		...
		public Integer get(Tool tool, Tree tree) {
			...
			if (getValue(tool) != null)
				val = Math.min(val == null ? Integer.MAX_VALUE : val, getValue(tool));
			if (getValue(tree) != null)
				val = Math.min(val == null ? Integer.MAX_VALUE : val, getValue(tree));
			return val;
		}
	};
	public static Option<Integer> MAX_HEIGHT = new Option<Integer>("Max Height", true, true, true, 2) {
		@Override
		public Integer load(Object o) {
			return loadInt(o);
		}
...
//		public DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree,
//				HashMap<Integer, ArrayList<Block>> blocks, Block block) {
//			if (Objects.equals(globalValue, true) || Objects.equals(toolValues.get(tool), true)
//					|| Objects.equals(treeValues.get(tree), true)) {
//				for (int x = -1; x <= 1; x++) {
//					for (int z = -1; z <= 1; z++) {
//						if (x == 0 && z == 0)
//							continue;
//						if (tree.trunk.contains(block.getRelative(x, 0, z).getType())) {
//							if (Objects.equals(globalValue, true))
...
//        }
//    };
	public static Option<Integer> MAX_HORIZONTAL_TRUNK_PILLAR_LENGTH = new Option<Integer>(
			"Max Horizontal Trunk Pillar Length", true, true, true, 6) {
		@Override
		public String getDesc(boolean ingame) {
		...
		protected DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree,
			...
			while (!xAxis.isEmpty()) {
				...
				do {
					...
					for (Block checking : xAxis) {
						if (checking.getX() == previous.getX() && checking.getY() == previous.getY()
								&& checking.getZ() == previous.getZ()) {
							b = checking;
				...
				do {
					...
					// dunno if Block is immutable, so I'll check it a more complicated way
					for (Block checking : xAxis) {
						if (checking.getX() == next.getX() && checking.getY() == next.getY()
								&& checking.getZ() == next.getZ()) {
							b = checking;
							stillPartOfTheTree = true;
							break;
						}
			...
			if (globalValue != null && actual > globalValue)
		...
		@Override
		public String[] getDebugText() {
			return generateDebugText("Found a horizontal trunk pillar that was too long$! {0}>{1}",
					"No excessive horizontal trunk pillars found");
		}
	};
						...
						for (int x = -1; x <= 1; x++) {
							for (int z = -1; z <= 1; z++) {
								...
								for (Block checking : theWholeTrunkSlice) {
									if (checking.getX() == newBlock.getX() && checking.getY() == newBlock.getY()
											&& checking.getZ() == newBlock.getZ()) {
										yep = true;
...
//    };
	public static Option<Integer> CUSTOM_MODEL_DATA = new Option<Integer>("Custom Model Data", true, true, true, null) {
		...
		public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
			...
			if (globalValue != null) {
				if (data != globalValue)
					return new DebugResult(this, GLOBAL, data, globalValue);
			}
			if (toolValues.containsKey(tool)) {
				if (data != toolValues.get(tool))
					return new DebugResult(this, TOOL, data, toolValues.get(tool));
			}
			if (treeValues.containsKey(tree)) {
				if (data != treeValues.get(tree))
					return new DebugResult(this, TREE, data, treeValues.get(tree));
			}
			return new DebugResult(this, SUCCESS);
		}
...
//        }
//    };
	public static OptionBoolean WITH_SNEAK = new OptionBoolean("With Sneak", true, true, true, true) {
		@Override
		public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
			if (player == null || !player.isSneaking())
				return null;
			if (Objects.equals(globalValue, false))
				return new DebugResult(this, GLOBAL);
			if (Objects.equals(toolValues.get(tool), false))
				return new DebugResult(this, TOOL);
			if (Objects.equals(treeValues.get(tree), false))
				return new DebugResult(this, TREE);
			return new DebugResult(this, SUCCESS);
		}
		...
		@Override
		public String[] getDebugText() {
			return generateDebugText("TreeFeller is disabled when sneaking", "Tool is disabled when sneaking",
					"Tree is disabled when sneaking", "Felling allowed when sneaking");
		}
	...
	public static OptionBoolean WITHOUT_SNEAK = new OptionBoolean("Without Sneak", true, true, true, false) {
		@Override
		public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
			if (player == null || player.isSneaking())
				return null;
...
//    };
	 ...
	 * This object is the result of <code>getConfig().get(getGlobalName())</code>
	 * for global options, or
	 * <code>list.get(getLocalName()) for local options</code>
	 * 
	...
	public final DebugResult check(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
		if (globalValue == null && !treeValues.containsKey(tree) && !toolValues.containsKey(tool))
			return null;
	...
	public boolean hasDebugText() {
		return getGlobalDebugText() != null || getToolDebugText() != null || getTreeDebugText() != null
				|| getSuccessDebugText() != null;
	}
	...
	private static String[] generateDebugText(String globalWith$, String success) {
		return new String[] { globalWith$.replace("$", ""), globalWith$.replace("$", " for tool"),
				globalWith$.replace("$", " for tree"), success };
	}
